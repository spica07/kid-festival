...
description: 꼬마 축제 페이지의 행사 데이터(assets/data/festivals.js)를 최신으로 유지한다. (0) 공공데이터를 그 시점 최신본으로 새로 받고, (1) 기존 행사 일정·장소를 웹 검색으로 다시 확인해 갱신하며(1개월 이내 임박 행사는 언론 교차검증 필수, 기사는 년도를 필수로 확인 — 표준데이터가 작년 정보를 담고 있을 수 있음), (2) 현재 월을 포함한 4개월간 서울·경기·인천의 신규 어린이 행사/축제를 검색해 추가한다. "축제 페이지 업데이트", "축제 일정 갱신", "신규 행사 추가" 등의 요청에 사용.
...
5. 이 절차는 `check-festival-dates` 스킬 / `festival-date-checker` 에이전트와 같은 방법론이다. 임박 행사가 많으면(예: 한 달치 전체 재검증) 그쪽 스킬로 별도 라운드를 돌리는 것도 방법이다 — 단 한 번에 너무 많은 행사를 병렬 웹서치로 몰아넣으면 세션 사용량 한도에 먼저 걸릴 수 있으니 배치 크기를 조절한다.
